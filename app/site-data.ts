export const scholar = "https://scholar.google.com/citations?user=vJ7KvzkAAAAJ&hl=en";
export const linkedin = "https://www.linkedin.com/in/yiming-liu-558a7198";

export const researchAreas = [
  {
    number: "01",
    title: "Electrical modulation",
    text: "Using electric fields and electrochemical driving forces to control ion transport, mineral nucleation, and scale formation at water interfaces.",
    tags: ["Electric fields", "Electrochemistry", "Mineral scaling"],
    publications: [
      { authorship: "First-Authored", title: "Mitigating CaCO₃ crystal nucleation and growth through continuous ion displacement via alternating electric fields", href: "https://doi.org/10.1038/s41467-024-55176-z" },
      { authorship: "First-Authored", title: "Monovalent-selective electrodialysis for lithium extraction from brine: role of operating conditions and water chemistry", href: "https://aiche.confex.com/aiche/icom26/meetingapp.cgi/Paper/727832" },
    ],
  },
  {
    number: "02",
    title: "Materials modulation",
    text: "Designing functional sorbents, membranes, and interfaces that tune selectivity and transport for recovery of critical minerals from complex waters.",
    tags: ["Functional materials", "Rare earth elements", "Selective transport"],
    publications: [
      { authorship: "First-Authored", title: "Selective extraction of rare earth elements using organophosphorus ligand modified metal organic frameworks", href: "https://aeesp2025.exordo.com/programme/presentation/210" },
      { authorship: "First-Authored", title: "Conducting thermal energy to the membrane/water interface for the enhanced desalination of hypersaline brines using membrane distillation", href: "https://doi.org/10.1016/j.memsci.2021.119188" },
    ],
  },
  {
    number: "03",
    title: "Circular water separations",
    text: "Integrating membrane, electrochemical, and thermal separations to recover water and resources from brines while minimizing waste and energy demand.",
    tags: ["High-recovery treatment", "Resource recovery", "Process intensification"],
    publications: [
      { authorship: "First-Authored", title: "Multistage surface-heated vacuum membrane distillation process enables high water recovery and excellent heat utilization", href: "https://doi.org/10.1021/acs.est.2c07094" },
      { authorship: "First-Authored", title: "Desalinating a real hyper-saline pre-treated produced water via direct-heat vacuum membrane distillation", href: "https://doi.org/10.1016/j.watres.2022.118503" },
    ],
  },
];

export const publications = [
  {
    year: "2025",
    journal: "AEESP 2025",
    title: "Selective extraction of rare earth elements using organophosphorus ligand modified metal organic frameworks",
    note: "Materials modulation",
    href: "https://aeesp2025.exordo.com/programme/presentation/210",
  },
  {
    year: "2026",
    journal: "ICOM 2026",
    title: "Monovalent-selective electrodialysis for lithium extraction from brine: role of operating conditions and water chemistry",
    note: "Electrical modulation",
    href: "https://aiche.confex.com/aiche/icom26/meetingapp.cgi/Paper/727832",
  },
  {
    year: "2025",
    journal: "Nature Communications",
    title: "Mitigating CaCO₃ crystal nucleation and growth through continuous ion displacement via alternating electric fields",
    note: "Electrical modulation",
    href: "https://doi.org/10.1038/s41467-024-55176-z",
  },
  {
    year: "2022",
    journal: "Water Research",
    title: "Desalinating a real hyper-saline pre-treated produced water via direct-heat vacuum membrane distillation",
    note: "Circular water separations",
    href: "https://doi.org/10.1016/j.watres.2022.118503",
  },
  {
    year: "2023",
    journal: "Environmental Science & Technology",
    title: "Multistage surface-heated vacuum membrane distillation process enables high water recovery and excellent heat utilization",
    note: "Circular water separations",
    href: "https://doi.org/10.1021/acs.est.2c07094",
  },
  {
    year: "2021",
    journal: "Journal of Membrane Science",
    title: "Conducting thermal energy to the membrane/water interface for the enhanced desalination of hypersaline brines using membrane distillation",
    note: "Materials modulation",
    href: "https://doi.org/10.1016/j.memsci.2021.119188",
  },
];

export const education = [
  ["2023", "Ph.D., Civil & Environmental Engineering", "University of California, Los Angeles"],
  ["2019", "M.S., Civil & Environmental Engineering", "University of California, Los Angeles"],
  ["2018", "B.S., Environmental Engineering", "Tsinghua University"],
];

export const postdoctoralTraining = [
  ["2023–2026", "Rice Academy Postdoctoral Fellow", "Rice University"],
];

export const newsItems = [
  ["2026", "Founding President Best Paper Award", "Chinese-American Professors in Environmental Engineering and Science"],
  ["2025", "Outstanding Oral Presentation Award", "19th Chinese Environmental Conference"],
  ["2025", "New work in Nature Communications", "Electrical control of CaCO3 nucleation and crystal growth"],
];

export const awards = [
  ["2026", "Founding President Best Paper Award", "Chinese-American Professors in Environmental Engineering and Science"],
  ["2025", "Outstanding Oral Presentation Award", "19th Chinese Environmental Conference"],
  ["2023", "Rice Academy Postdoctoral Fellowship", "Rice University"],
  ["2023", "University Fellowship", "UCLA"],
  ["2021", "2nd Place, Internet of Water De Nora Idea Challenge", "Water innovation competition"],
  ["2017–18", "Academic Excellence Award", "Tsinghua University"],
];

export const profileLinks = [
  ["LinkedIn", linkedin],
  ["Google Scholar", scholar],
  ["ORCID", "https://orcid.org/0000-0003-4761-5531"],
];
